...
var Main = function() {
	...
	function notifyMe() {
		  ...
		  if (!("Notification" in window)) {
		    ...
		    var options = {
		    	      body: "We've updated the system with some exciting features ! v" + ebsVersion,
		    	      icon: "assets/logo/"+companyCode+".png"
		    	  }
		    	  var n = new Notification("CTrends EBS",options);
		...
		$.ajax("/loadpref")
			.done(function(data){
				if (data.theme == null) {
					...
					appSetting.fixedHeader = data.fixedHeader;
					appSetting.fixedSidebar = data.fixedSidebar;
					appSetting.closedSidebar = data.closedSidebar;
					appSetting.fixedFooter = data.fixedFooter;
					appSetting.theme = data.theme;
					appSetting.initialNotification = data.initialNotification;
					
					if(appSetting.initialNotification === true){
						notifyMe();
						//toastr.success("We've updated the system with some exciting features !", 'CTrends EBS'); 	
						appSetting.initialNotification = false;   
						$.ajax({
							url: "/savepref",
							data: appSetting
						});
